  ...
  if (message.channel.id !== client.config.channels.generator) return message.channel.sendErrorMessage(`This command can only be used in <#${client.config.channels.generator}>`)
    fs.readFile("./accounts/instagram", (err, file) => {
        ...
        fs.readFile('./accounts/instagram', 'utf8', function(err, data) {
            ...
            fs.writeFile('./accounts/instagram', lines.slice(1).join('\n'), function(err) {
                if(err) throw err;
            });
...
module.exports.help = {
    ...
    category: 'generator',
    description: "generate a instagram account",
    cooldown: 900,
    usage: '',
    example: [],
};
