  ...
  if (message.channel.id !== client.config.channels.generator) return message.channel.sendErrorMessage(`This command can only be used in <#${client.config.channels.generator}>`)
    fs.readFile("./accounts/xbox.txt", (err, file) => {
        ...
        fs.readFile('./accounts/xbox.txt', 'utf8', function(err, data) {
            ...
            fs.writeFile('./accounts/xbox.txt', lines.slice(1).join('\n'), function(err) {
                if(err) throw err;
            });
...
module.exports.help = {
    ...
    aliases: ['xbox'],
    category: 'generator',
    description: "generate an xbox account",
    cooldown: 900,
    usage: '',
    example: [],
};